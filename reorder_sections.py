...
if formations_match and solutions_match:
    formations_html = formations_match.group(1)
    solutions_html = solutions_match.group(1)
    
    # Remove them from current positions
    html = html.replace(formations_html, '')
    html = html.replace(solutions_html, '')
    if targets_match:
        targets_html = targets_match.group(1)
        html = html.replace(targets_html, '')
        
        # Insert them in the new order: Formations -> Targets -> Solutions
        # Find the place where solutions used to be
        insert_marker = '<!-- ACCOMPAGNEMENT SUBSECTION -->' # Not good. We'll find problems end
        
        # We can reconstruct it simply:
        # After <section id="problems">...</section>
        
        parts = html.split('</section>')
        # Reconstruct, looking for problems
        new_html = ""
        for part in parts:
            new_html += part + '</section>'
            if '<section id="problems"' in part:
                new_html += '\n\n' + formations_html + '\n\n' + targets_html + '\n\n' + solutions_html
                
        # Remove trailing artifact from split
        if new_html.endswith('</section></section>'):
            new_html = new_html[:-10]
            
        with open('index_swapped.html', 'w', encoding='utf-8') as f:
            f.write(new_html)
        print("Successfully rebuilt index.html order!")
